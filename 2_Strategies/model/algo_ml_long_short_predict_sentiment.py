...
class MyStrategy(StrategyTemplate):
    ...
    def __init__(self):
        ...
        self.model = load_model('model_long_short_predict_sentiment.h5')
        ...
        # Loop to create 'repeatCount' different sentiment_score indicators, each with a different timeperiod
        for a in range(0, self.repeatCount):
            tp = (a + 1) * self.repeatStep + 1  # Calculate the time period for the current sentiment_score
            self.hData.append("sentiment_score" + str(tp))  # Add sentiment_score column name to header
            # Add ROC indicator to the strategy, linked to the main data feed
            self.sentiment_score.append(bt.talib.SMA(self.data, timeperiod=tp, plot=False))
    ...
    def next(self):
        ...
        for a in range(0, self.repeatCount):
         ...
         # Collect sentiment_score values, handling NaN by using 0
        for a in range(0, self.repeatCount):
            if math.isnan(self.sentiment_score[a][0]): # Check if the sentiment_score value is NaN
                inputRec.append(0)         # If NaN, use 0 as a fallback
            else:
                inputRec.append(self.sentiment_score[a][0]) # Otherwise, use the calculated sentiment_score value
